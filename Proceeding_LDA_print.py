...
exclude = ['-', 'et', 'al', 'B.V.', 'LLC','L.L.C', 'LLP', 'INC', 'Inc','S.r.l', 'S.a.S', 'called', 'would', 'have', 'See Id', 'Id']
...
def Corpus(Case_file_path, corpus, word_filter):
    ...
    for file in path_list:
        ...
        for sentence in tokenized:
            ...
            filtered_sentence = [x for x in lemmatized_sentence if x not in word_filter]
...
def Proceeding_TFcv(n_features, split_ratio, data, stopwords):
...
# Ingredients for LDA
saved_path = "/Users/osuhyeon/NLTK_court_document/1 LDA Perplexities/"
perplexity_str = open(saved_path + "Case%d.txt" % int(i), 'r').read()
perplexity_dict = dict(item.split(":") for item in perplexity_str.split(","))
...
lda, lda_train = Proceeding_LDA(
    n_component = n_component, 
    ngram_tf_train = ngram_tf_train)

saveLDAnumpy = "/Users/osuhyeon/NLTK_court_document/1 LDA Numpy/" 
...
saveTextDictionary = "/Users/osuhyeon/NLTK_court_document/1 LDA Text Dictionary/"
with open(saveTextDictionary + "Case%d" % int(i), 'w', encoding="UTF8") as f:
